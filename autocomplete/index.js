...
class AutocompletePrompt extends Base {
  constructor(
    ...
    if (!this.opt.searchIn) {
      this.throwParamError('searchIn');
    }
    this.currentChoices = [];
  ...
  ara(input, dictionary) {
    input = input || ''
    return new Promise(function (resolve) {
        setTimeout(function () {
            var fuzzyResult = fuzzy.filter(input, Array.from(dictionary.entries()), {
  ...
  search(searchTerm /* : ?string */) {
    ...
    var thisPromise = self.ara(searchTerm, self.opt.searchIn);
